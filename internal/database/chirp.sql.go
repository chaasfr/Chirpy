...
const getAllChirpFromUser = `-- name: GetAllChirpFromUser :many
SELECT id, created_at, updated_at, body, user_id from chirps where user_id = $1 order by created_at asc
`

func (q *Queries) GetAllChirpFromUser(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChirpFromUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
